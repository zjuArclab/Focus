...
def smulw(ir, instr, a, b, c):
    ...
    print "b",b
    print "c",c
    print "instr.name[5])",instr.name[5]
...
def stcgtl(ir, instr, a, b, c):
    pass

def ldc(ir, instr, a = 0, b = 0, c = 0, d = 0, e = 0, f = 0):
    pass
...
def is_pc_written(ir, instr_ir):
    ...
    if isinstance(instr_ir, Iterable):
        for ir in instr_ir:
            if ir.dst in all_pc:
                return True, ir.dst
    return False, None
...
def get_mnemo_expr(ir, instr, *args):
    ...
    try:
        ...
        if ir is None or instr is None or args is None:
            with open('G:\\test.txt', 'w') as fp:
                fp.write(str(cond) + ' ' + str(mf) + ' ' + str(instr.name.lower()) + '\n')
        instr_ir = None
        extra_ir = None
...
class ir_arml(IntermediateRepresentation):
    ...
    def mod_pc(self, instr, instr_ir, extra_ir):
        ...
        if isinstance(extra_ir, Iterable):
    ...
    def get_ir(self, instr):
        ...
        if not instr_ir is None and not extra_ir is None:
...
class ir_armtl(ir_arml):
    ...
    def mod_pc(self, instr, instr_ir, extra_ir):
        ...
        if not isinstance(instr_ir, Iterable):
            for i, expr in enumerate(instr_ir):
                dst, src = expr.dst, expr.src
                if dst != self.pc:
                    dst = dst.replace_expr(pc_fixed)
                src = src.replace_expr(pc_fixed)
                instr_ir[i] = ExprAff(dst, src)
        if not isinstance(extra_ir, Iterable):
            for idx, irblock in enumerate(extra_ir):
                extra_ir[idx] = irblock.modify_exprs(lambda expr: expr.replace_expr(pc_fixed) \
                                                     if expr != self.pc else expr,
                                                     lambda expr: expr.replace_expr(pc_fixed))
