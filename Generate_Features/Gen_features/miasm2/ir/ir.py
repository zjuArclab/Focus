...
class AssignBlock(object):
    ...
    def __setitem__(self, dst, src):
        print "AssignBlock is immutable"
...
class IntermediateRepresentation(object):
    ...
    def get_ir(self, instr):
        print "CANNOT ANALYSIS INST  :  ",instr
        instr = "NOP"
        return self.get_ir(instr)
    ...
    def add_block(self, block, gen_pc_updt=False):
        ...
        for instr in block.lines:
            ...
            # if "BREAK" in str(instr):
            #     instr = "NOP"
            if str(instr.name).lower()== "brk":
                continue
            split = self.add_instr_to_irblock(block, instr, assignments, ir_blocks_all, gen_pc_updt)
            if split:
